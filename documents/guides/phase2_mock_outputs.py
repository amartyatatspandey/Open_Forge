...
"""
Mock Phase 2 outputs for Phase 3 + 4 development without running real Phase 2.

These fixtures represent realistic GridMatrix outputs that Phase 2 would produce
on the 5 golden datasheets. They cover:
- Different table sizes and shapes
- Merged cells (colspan/rowspan)
- Empty cells and sparse data
- Mixed content (text + numbers)
- Edge cases (single-row headers, wide tables)

Later, when Phase 2 is run on a real GPU, replace these mocks with actual
Phase2Output. The Phase 3 code will work unchanged — it only depends on the
GridMatrix schema being correct, which these mocks follow exactly.
"""

from src.schemas.pipeline import GridMatrix, Phase2Output
from pathlib import Path


def mock_gridmatrix(
    rows: list[list[str]],
    section_type: str = "electrical_characteristics",
    ...
    # Pad ragged rows to consistent column count
...
# ============================================================================
# TI_SN74LVC1G04 — Logic IC, simple, clean bordered tables
# ============================================================================

def mock_sn74_electrical_grid() -> GridMatrix:
    """Electrical characteristics table — 4 parameters, standard format."""
...
def mock_sn74_pinout_grid() -> GridMatrix:
    """Pinout table — pin definitions (simple 4-pin package)."""
    return mock_gridmatrix(
        rows=[
...
def mock_sn74_absolute_max() -> GridMatrix:
    """Absolute maximum ratings — stress limits."""
    return mock_gridmatrix(
        rows=[
            ["Parameter", "Max", "Unit"],
            ["V_CC", "6.0", "V"],
...
def mock_sn74_phase2_output() -> Phase2Output:
    """Complete Phase 2 output for SN74LVC1G04."""
    return Phase2Output(
        grids=[
        ...
        metadata={
            "pdf_path": "corpus/golden/TI_SN74LVC1G04_v1.pdf",
            "component_id": "SN74LVC1G04",
        }
    )
...
def mock_tlv7021_electrical_grid() -> GridMatrix:
    """Electrical characteristics — 7 parameters, some with sparse data."""
    return mock_gridmatrix(
        rows=[
            ["Parameter", "Min", "Typ", "Max", "Unit", "Conditions"],
            ["V_CC", "2.0", "3.3", "5.5", "V", "—"],
        ...
        section_type="electrical_characteristics",
        confidence=0.92,  # Lower confidence for VLM-extracted borderless table
        source="vlm_path_B"
    )


def mock_tlv7021_pinout_grid() -> GridMatrix:
    """Pinout — 8-pin package with multiple functions per pin."""
    return mock_gridmatrix(
        rows=[
...
def mock_tlv7021_phase2_output() -> Phase2Output:
    """Complete Phase 2 output for TLV7021."""
    return Phase2Output(
        grids=[
...
# ============================================================================
# TI_INA219 — I2C current sensor, timing diagrams, many parameters
# ============================================================================

def mock_ina219_electrical_grid() -> GridMatrix:
    """Electrical characteristics — 12 parameters."""
    return mock_gridmatrix(
        rows=[
            ["Parameter", "Min", "Typ", "Max", "Unit", "Conditions"],
            ["V_BUS_MAX", "—", "26", "—", "V", "—"],
        ...
        section_type="electrical_characteristics",
        confidence=0.88,  # More sparse, harder table
        source="vlm_path_B"
    )


def mock_ina219_pinout_grid() -> GridMatrix:
    """Pinout — 16-pin SOIC package."""
    return mock_gridmatrix(
        rows=[
            ["Pin", "Name", "Type", "Description"],
...
def mock_ina219_phase2_output() -> Phase2Output:
    """Complete Phase 2 output for INA219."""
    return Phase2Output(
        grids=[
        ...
        metadata={
            "pdf_path": "corpus/golden/TI_INA219_v1.pdf",
            "component_id": "INA219",
        }
    )
...
def mock_lm5176_electrical_grid() -> GridMatrix:
    """Electrical characteristics with conditional parameters."""
    return mock_gridmatrix(
        rows=[
...
def mock_lm5176_pinout_grid() -> GridMatrix:
    """Pinout — 16-pin package with complex pin functions."""
    return mock_gridmatrix(
        rows=[
...
def mock_lm5176_phase2_output() -> Phase2Output:
    """Complete Phase 2 output for LM5176."""
    return Phase2Output(
        grids=[
        ...
        metadata={
            "pdf_path": "corpus/golden/TI_LM5176_v1.pdf",
            "component_id": "LM5176",
            "footnote_map": {
...
# ============================================================================
# TI_TPS62933 — Buck converter, modern datasheet with sparse tables
# ============================================================================

def mock_tps62933_electrical_grid() -> GridMatrix:
    """Electrical characteristics — modern TI style, some sparse cells."""
    return mock_gridmatrix(
        rows=[
            ["Parameter", "Min", "Typ", "Max", "Unit", "Conditions"],
            ["V_IN", "2.7", "—", "6.0", "V", "—"],
...
def mock_tps62933_phase2_output() -> Phase2Output:
    """Complete Phase 2 output for TPS62933."""
    return Phase2Output(
        grids=[
        ...
        metadata={
            "pdf_path": "corpus/golden/TI_TPS62933_v1.pdf",
...
# ============================================================================
# Convenience: All phase 2 outputs in one place
# ============================================================================

def all_golden_phase2_outputs() -> dict[str, Phase2Output]:
    """All 5 golden Phase 2 outputs for Phase 3 + 4 testing."""
    return {
...
# ============================================================================
# Usage example (for Phase 3 development)
# ============================================================================

if __name__ == "__main__":
    outputs = all_golden_phase2_outputs()
    for component_id, phase2_out in outputs.items():
        print(f"\n{component_id}:")
        for grid in phase2_out.grids:
            print(f"  {grid.section_type:35s} {grid.num_rows}r × {grid.num_cols}c")
